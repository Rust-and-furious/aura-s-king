...
impl Useable for Lit {
    fn utiliser(&mut self, player: &mut Player, world: &mut WorldManager) -> Result<(), &'static str> {
        ...
        if chance < 50 {
            ...
            println!("{} Rêve héroïque : vous vous voyez en armure étincelante sur un blanc destrier.", colore!(Vert, "[+10 000 Aura]"));
        } else {
            player.aura -= 30000.0;
            crate::audio::play_sound("assets/defeat.wav");
            println!("{} Cauchemar de paysan : vous rêvez que vous êtes un chou cultivé et récolté par vous-même.", colore!(Rouge, "[-30 000 Aura]"));
        }
        Ok(())
    }
}
